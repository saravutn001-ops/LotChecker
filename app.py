...
def normalize_image_base64_for_ai(image_base64, max_side=1400):
    """
...
def enhance_lot_image_for_ai(image_base64, check_type=""):
    ...
    """
    try:
        clean_b64 = normalize_image_base64_for_ai(image_base64, max_side=2200)
...
"""
    ...
    if engine == "dual":
        # When checking 2 pouch machines + carton, running both OCR engines for every pouch can
        # create 5-6 external API calls and Render may return an HTML timeout page.
        # Default: use a single fast engine for pouch images, keep dual/selection for cartons.
        if check_type == "pouch" and str(os.getenv("DUAL_POUCH", "0")).strip() != "1":
            try:
...
def handle_request_too_large(e):
    return jsonify({"error": "รูปภาพรวมมีขนาดใหญ่เกินไป กรุณาถ่ายใหม่หรืออัปโหลดรูปที่เล็กลง"}), 413
...
def handle_internal_error(e):
    if request.path == "/check":
        return jsonify({"error": "ระบบหลังบ้านประมวลผลไม่สำเร็จ กรุณาลองใหม่ หรือดู Render Logs"}), 500
    return e
