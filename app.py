...
WORK_ORDER_FILE = os.getenv("WORK_ORDER_FILE", "work_orders.json")
...
def save_work_orders(data):
    folder = os.path.dirname(os.path.abspath(WORK_ORDER_FILE))
    if folder:
        os.makedirs(folder, exist_ok=True)
    tmp = WORK_ORDER_FILE + ".tmp"
    with open(tmp, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)
    os.replace(tmp, WORK_ORDER_FILE)
...
<style>
body{font-family:Arial,Tahoma,sans-serif;background:#eef4fb;margin:0;padding:12px;color:#0f172a}
.app{max-width:1180px;margin:0 auto}
.header{background:#071f38;color:#fff;border-radius:18px;padding:14px 16px;margin-bottom:12px}
.header h1{margin:0;font-size:22px}.header p{margin:5px 0 0;color:#cbd5e1}
.card{background:#fff;border:1px solid #dbe4ef;border-radius:18px;padding:14px;margin-top:12px;box-shadow:0 6px 18px rgba(15,23,42,.06)}
.grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}
.field label{display:block;font-weight:700;font-size:13px;color:#475569;margin-bottom:6px}
input,select{width:100%;height:44px;border:1px solid #dbe4ef;border-radius:12px;padding:0 12px;font-size:15px}
select[multiple]{height:auto;min-height:44px;padding:8px 12px}
button,a.btn{border:0;border-radius:14px;padding:13px 14px;font-size:16px;font-weight:800;color:#fff;background:#0b63ce;cursor:pointer;text-align:center;text-decoration:none;display:inline-block}
...
table{width:100%;border-collapse:collapse;font-size:14px}th,td{border-bottom:1px solid #e2e8f0;padding:8px;text-align:left;vertical-align:top}th{background:#f8fafc}
.status{margin-top:10px;font-weight:800}
.small{font-size:12px;color:#64748b}
@media(max-width:720px){.grid{grid-template-columns:1fr}.actions{grid-template-columns:1fr}table{font-size:12px}}
</style>
</head>
<body>
<div class="app">
  <div class="header">
    ...
    <p>หัวหน้างานกำหนดรูปแบบล็อตไว้ล่วงหน้า พนักงานใส่ Work Order แล้วถ่ายรูปตรวจเท่านั้น</p>
  </div>

  <div class="card">
    <div class="grid">
      <div class="field"><label>รหัสผ่าน Admin</label><input id="adminPassword" type="password" placeholder="ค่าเริ่มต้น 1234"></div>
      <div class="field"><label>&nbsp;</label><button type="button" onclick="loadList()">โหลดรายการ WO</button></div>
      <div class="field"><label>&nbsp;</label><a class="btn" href="/">กลับหน้า ตรวจล็อต</a></div>
    </div>
  ...
  <div class="card">
    ...
    <div class="grid">
      <div class="field"><label>Work Order</label><input id="wo" placeholder="WO25070001"></div>
      <div class="field"><label>ประเภทไลน์</label><select id="mode" onchange="refreshMachines()"><option value="">เลือกประเภทไลน์</option><option value="linapack">Linapack</option><option value="sachet">Sachet</option><option value="auto">Auto</option></select></div>
      <div class="field"><label>เครื่องซองที่ 1</label><select id="line"></select></div>
      <div class="field"><label>เครื่องซองเพิ่มเติม</label><select id="extraLines" multiple onchange="updatePreview()"></select><div class="small">เลือกได้มากกว่า 1 เครื่อง ถ้าไม่มีให้เว้นว่าง</div></div>

      <div class="field"><label>ผลิตภัณฑ์</label><select id="productType" onchange="updateCalculated()"><option value="">เลือกผลิตภัณฑ์</option><option value="EPC">EPC</option><option value="EPW">EPW</option><option value="FS">FS</option><option value="IS">IS</option><option value="SS">SS</option></select></div>
      <div class="field"><label>ประเภทงาน</label><select id="marketType" onchange="updateMarketUI()"><option value="">เลือกประเภทงาน</option><option value="TH">งานไทย</option><option value="EXPORT">งานต่างประเทศ</option><option value="LAOS">งานต่างประเทศ ลาว</option></select></div>
      <div class="field"><label>อายุ EPC งานลาว</label><select id="epcLaosShelfLifeMonths" onchange="updateCalculated()"><option value="24">2 ปี</option><option value="15">1 ปี 3 เดือน</option></select></div>
      <div class="field"><label>วันที่ผลิต</label><input id="mfgDate" type="date" onchange="updateCalculated()"></div>

      <div class="field"><label>MFG</label><input id="mfg" readonly></div>
      <div class="field"><label>EXP</label><input id="exp" readonly placeholder="ระบบคำนวณ"></div>
      <div class="field"><label>วันที่ผสม</label><input id="mixDate" type="date" onchange="updateMixCode()"></div>
      <div class="field"><label>Mix Code</label><input id="mixCode" readonly placeholder="ระบบคำนวณ"></div>
      <div class="field"><label>Prefix กล่อง</label><select id="cartonPrefix" onchange="updateShippingMark()"></select></div>

      <div class="field"><label>Shipping Mark</label><input id="shippingMark" readonly></div>
      <div class="field"><label>เลขอาคาร</label><select id="buildingNo"><option value="">เลือกเลขอาคาร</option><option>1</option><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option></select></div>
      <div class="field"><label>Suffix</label><input id="buildingSuffix" placeholder="เช่น QR / N"></div>
      <div class="field"><label>สถานะ</label><select id="active"><option value="true">ใช้งาน</option><option value="false">ปิดใช้งาน</option></select></div>
    </div>
...
function refreshMachines(){
  const sel=$("line"), old=sel.value, mode=$("mode").value;
  sel.innerHTML="";
  sel.appendChild(new Option("เลือกเครื่อง", ""));
  machineListForMode(mode).forEach(x=>sel.appendChild(new Option(x,x)));
  if(machineListForMode(mode).includes(old)) sel.value=old;
  refreshExtraMachines();
  updateCalculated();
}
function refreshExtraMachines(selectedValues){
  const sel=$("extraLines");
  if(!sel) return;
  const mode=$("mode").value;
  const selected = Array.isArray(selectedValues) ? selectedValues : Array.from(sel.selectedOptions || []).map(o=>o.value);
  sel.innerHTML="";
  machineListForMode(mode).forEach(x=>{
    const opt = new Option(x, x);
    opt.selected = selected.includes(x);
    sel.appendChild(opt);
  });
}
function ddmmyyFromDate(v){
...
function updateCalculated(){
  $("mfg").value = ddmmyyFromDate($("mfgDate").value);
...
function updateShippingMark(){
  ...
  if(market==="TH") $("shippingMark").value="ไม่ตรวจ";
  else $("shippingMark").value=PREFIX_SHIPPING_MAP[p] || "ไม่ตรวจ";
  updatePreview();
}
function getPayload(){
  const primaryLine = $("line").value;
  const extras = Array.from($("extraLines")?.selectedOptions || []).map(o=>o.value).filter(x=>x && x !== primaryLine);
  const pouches = [primaryLine].concat(extras).filter(Boolean).map(line=>({line}));
  return {
...
function updatePreview(){
  ...
  $("preview").innerHTML = `ตัวอย่าง: WO <b>${p.workOrder || "-"}</b> | ${lines || "-"} | MFG ${p.mfg || "-"} | EXP ${p.exp || "ไม่ต้องมี/ระบบไม่ตรวจ"} | กล่อง Prefix ${p.cartonAlphaCode || "-"} อาคาร ${p.buildingNo || "-"}`;
}
async function saveWO(){
  ...
  if((p.marketType==="EXPORT" || p.marketType==="LAOS") && !p.cartonAlphaCode) required.push("Prefix");
...
async function loadList(){
  ...
  const data=await res.json();
  if(!res.ok) return setStatus(data.error || "โหลดรายการไม่สำเร็จ", true);
...
function renderList(list){
  ...
  $("list").innerHTML = `<table><thead><tr><th>WO</th><th>ตั้งค่า</th><th>Lot ที่ควรเป็น</th><th>สถานะ</th><th>แก้ไข</th></tr></thead><tbody>`+
    list.map(w=>`<tr><td><b>${w.workOrder}</b></td><td>${w.mode} / ${(w.pouches||[]).map(x=>x.line).join(", ")}<br>${w.productType} / ${w.marketType}<br>MFG ${w.mfg}</td><td>ซอง: ${w.expectedPouchLot || "-"}<br>กล่อง: ${w.expectedCartonLot || "-"}</td><td>${w.active ? "ใช้งาน" : "ปิด"}</td><td><button type="button" onclick='editWO(${JSON.stringify(w).replace(/'/g,"&#39;")})'>โหลดแก้ไข</button></td></tr>`).join("")+
    `</tbody></table>`;
}
function editWO(w){
  ...
  refreshExtraMachines(pouches.slice(1).map(x=>x.line));
...
async function deleteWO(){
  const wo=$("wo").value.trim().toUpperCase(), password=$("adminPassword").value;
...
function clearForm(){
  ["wo","mfgDate","mfg","exp","mixDate","mixCode","shippingMark","buildingSuffix"].forEach(id=>$(id).value="");
  ["mode","line","productType","marketType","cartonPrefix","buildingNo"].forEach(id=>$(id).value="");
  $("epcLaosShelfLifeMonths").value="24"; $("active").value="true";
  refreshMachines(); updateMarketUI(); updatePreview();
}
function setStatus(msg,isErr){$("status").textContent=msg;$("status").style.color=isErr?"#dc2626":"#166534";}
window.addEventListener("DOMContentLoaded",()=>{refreshMachines();updateMarketUI();updateCalculated();});
</script>
</body>
</html>"""
...
def api_work_orders():
    ...
    if market_type in ["EXPORT", "LAOS"] and not str(payload.get("cartonAlphaCode", payload.get("cartonPrefix", "")) or "").strip():
...
def enrich_work_order(wo):
    ...
    if market_type == "TH":
        wo["expectedCartonLot"] = f"00001 00 {mfg} {building_no}{(' ' + suffix) if suffix else ''}".strip()
        wo["cartonAlphaCode"] = "00"
        wo["shippingMark"] = ""
    else:
