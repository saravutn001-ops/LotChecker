...
function renderList(list){
  ...
  $("list").innerHTML = `<div class="table-wrap"><table class="wo-table"><thead><tr><th>WO</th><th>ตั้งค่า</th><th>Lot ที่ควรเป็น</th><th>สถานะ</th><th>แก้ไข</th></tr></thead><tbody>`+
    list.map((w,i)=>`<tr><td><b>${w.workOrder}</b></td><td>${w.mode} / ${(w.pouches||[]).map(x=>x.line).join(", ")}<br>${w.productType} / ${w.marketType}<br>MFG ${w.mfg}</td><td>ซอง: ${w.expectedPouchLot || "-"}<br>กล่อง: ${w.expectedCartonLot || "-"}</td><td><span class="pill">${w.active ? "ใช้งาน" : "ปิด"}</span></td><td><button type="button" class="btn-small" onclick="editWOByIndex(${i})">โหลดแก้ไข</button></td></tr>`).join("")+
    `</tbody></table></div>`;
}
