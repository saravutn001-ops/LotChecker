...
def vt_carton_no_exp_required(market_type, carton_alpha_code=""):
    """VN-MT / Prefix VT carton format does not contain EXP."""
    return (
        str(market_type or "").strip().upper() == "EXPORT"
        and str(carton_alpha_code or "").strip().upper() == "VT"
    )
...
def api_work_orders():
    ...
    # Linapack mix-date validation. VN-MT / Prefix VT does not require Mix Code.
...
def _check_sync_from_data(data):
    try:
        ...
        if check_type == "pouch" and len(pouches) > 1:
            ...
            carton_check_exp = (original_market_type != "LAOS") and not vt_carton_no_exp_required(original_market_type, carton_alpha_code)
