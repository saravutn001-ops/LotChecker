...
def _check_sync_from_data(data):
    try:
        ...
        stamped_filename = stamp_image(
            ...
            (pouches[1].get("image", "") if len(pouches) > 1 else None),
            [item.get("image", "") for item in pouches[1:]] if check_type in ["pouch", "both"] and len(pouches) > 1 else None
        )
        ...
        if check_type in ["pouch", "both"] and len(pouches) > 1:
