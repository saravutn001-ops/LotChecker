...
def enhance_lot_image_for_ai(image_base64, check_type=""):
    ...
    try:
        ...
        target_min_w = 1400 if str(check_type).lower() == "carton" else 1300
        if w < target_min_w:
            scale = min(1.8, target_min_w / max(1, w))
...
def check():
    try:
        ...
        if check_type == "both":
            # PARALLEL OCR FIX:
            # When checking 2-4 pouch machines + carton, doing OCR one-by-one can exceed
            # Render/Gunicorn timeout and the browser receives an HTML error page instead of JSON.
            # Run each image OCR concurrently, then keep the same strict validation logic.
            carton_base64 = normalize_image_base64_for_ai(carton_image_data)
            ...
            def ocr_one_pouch(idx, pouch_item):
                line_code = pouch_item.get("line", "").strip().upper()
                pouch_img = pouch_item.get("image", "")
                pouch_base64 = normalize_image_base64_for_ai(pouch_img)
            ...
            def ocr_carton():
                ...
                carton_json_local = json.loads(clean_json_text(raw_carton_ai))
                return carton_json_local, carton_json_local.get("lines", [])

            pouch_results = []
            carton_json = {}
            carton_lines = []
            max_workers = min(6, max(2, len(pouches) + 1))
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                future_map = {}
                for idx, pouch_item in enumerate(pouches, start=1):
                    future_map[executor.submit(ocr_one_pouch, idx, pouch_item)] = ("pouch", idx)
                future_map[executor.submit(ocr_carton)] = ("carton", 0)
                ...
                for future in as_completed(future_map):
                    ...
                    try:
                        ...
                        if kind == "pouch":
                            raise RuntimeError(f"OCR ซองเครื่อง {idx} ล้มเหลว: {friendly_error_message(e)}")
                        raise RuntimeError(f"OCR กล่องล้มเหลว: {friendly_error_message(e)}")
                    if kind == "pouch":
                        pouch_results.append(result)
                    else:
                        carton_json, carton_lines = result

            pouch_results.sort(key=lambda x: x.get("index", 0))
