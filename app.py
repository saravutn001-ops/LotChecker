...
<div class="app">
  ...
  <div class="card">
    ...
    <div class="grid">
      ...
      <div class="field"><label>เครื่องซองเพิ่มเติม</label><input id="extraLines" placeholder="เช่น LP5,LP6 หรือ เว้นว่าง"></div>
      ...
      <div class="field"><label>Mix Code</label><input id="mixCode" placeholder="เช่น 18F ถ้ามี"></div>
...
function getPayload(){
  const extras = $("extraLines").value.split(",").map(x=>x.trim()).filter(Boolean);
  const pouches = [$("line").value].concat(extras).filter(Boolean).map(line=>({line}));
  return {
...
function editWO(w){
  ...
  $("extraLines").value=pouches.slice(1).map(x=>x.line).join(",");
...
function clearForm(){
  ["wo","extraLines","mfgDate","mfg","exp","mixCode","shippingMark","buildingSuffix"].forEach(id=>$(id).value="");
