...
def linapack_requires_mix(product_type, market_type, carton_alpha_code=""):
    ...
    # Standard rule: EPW Thailand/Laos requires a mix-date code.
    # Special export rule: destination/prefix VT requires the mix-date code
    # for Linapack lots regardless of product type, e.g. MFG 080826 07H LP8 12:34.
    return (product_type == "EPW" and market_type in ["TH", "LAOS"]) or (market_type == "EXPORT" and carton_alpha_code == "VT")
...
def no_exp_required(product_type, market_type):
    ...
    EPW EXPORT  : MFG DDMMYY เลขเครื่อง เวลา (ยกเว้น Prefix VT ต้องมีวันผสม)
    EXPORT Prefix VT (Linapack ทุกผลิตภัณฑ์): MFG DDMMYY วันผสม เลขเครื่อง เวลา
...
function updateConditionalFields(){
  ...
  const showMix = mode === "linapack" && ((product === "EPW" && (market === "TH" || market === "LAOS")) || (market === "EXPORT" && prefix === "VT"));
...
function getPayload(){
  ...
  return {
    ...
    mixDate:($("mode").value==="linapack" && (($("productType").value==="EPW" && ["TH","LAOS"].includes($("marketType").value)) || ($("marketType").value==="EXPORT" && $("cartonPrefix").value==="VT"))) ? ddmmyyFromDate($("mixDate")?.value || "") : "",
    mixCode:($("mode").value==="linapack" && (($("productType").value==="EPW" && ["TH","LAOS"].includes($("marketType").value)) || ($("marketType").value==="EXPORT" && $("cartonPrefix").value==="VT"))) ? $("mixCode").value.trim().toUpperCase() : "",
    cartonAlphaCode:$("cartonPrefix").value,
