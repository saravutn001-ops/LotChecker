...
def normalize_epc_laos_exp_months(value):
    """
    EPC Laos has two allowed shelf-life rules depending on line/product spec:
    - 24 months (default / normal)
    - 15 months (special lines: 1 year 3 months)
    """
    text = str(value or "").strip().upper().replace(" ", "")
    if text in ["15", "15M", "1Y3M", "1Y3", "1ปี3เดือน"]:
        return 15
    if text in ["24", "24M", "2Y", "2", "2ปี"]:
        return 24
    try:
...
def check():
    try:
        ...
        return jsonify({
            ...
            "epcLaosShelfLifeMonths": epc_laos_exp_months if product_type == "EPC" and original_market_type == "LAOS" else "",
