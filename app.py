...
def stamp_image(image_base64, summary, check_type, product_type, market_type, mode, checked_time, carton_image_base64=None, pouch2_image_base64=None):
    """
    ...
    if carton_image_base64:
        ...
        if pouch2_image_base64:
            images.append(("POUCH 2", _open_base64_image(pouch2_image_base64)))
        images.append(("CARTON", _open_base64_image(carton_image_base64)))

        canvas_w = 2100 if len(images) == 3 else 1800
        header_h = 155
        footer_h = 170
        gap = 24
        margin = 36
    ...
    try:
        ...
        if check_type == "both":
            if not pouch_image_data or not carton_image_data:
                return jsonify({"error": "กรุณาถ่าย/อัปโหลดรูปซองเครื่องที่ 1 และรูปกล่องให้ครบ"}), 400
            if bool(pouch2_image_data) != bool(expected_line2):
                return jsonify({"error": "ถ้าตรวจซองเครื่องที่ 2 ต้องเลือกเครื่องที่ 2 และใส่รูปซองเครื่องที่ 2 ให้ครบ"}), 400
        elif not image_data:
        ...
        if check_type == "both":
            ...
            pouch2_json = None
            pouch2_lines = []
            if pouch2_image_data and expected_line2:
                pouch2_base64 = pouch2_image_data.split(",", 1)[1] if "," in pouch2_image_data else pouch2_image_data
                raw_pouch2_ai = read_lot_with_ai(
                    pouch2_base64, "pouch", mode, product_type, market_type, expected_mfg, expected_line2,
                    expected_exp, mix_code, building_no, building_suffix, shipping_mark, carton_alpha_code
                )
                pouch2_json = json.loads(clean_json_text(raw_pouch2_ai))
                pouch2_lines = pouch2_json.get("lines", [])
            ...
            pouch2_overall = True
            pouch2_details = []
            if pouch2_json is not None:
                if mode == "sachet":
                    pouch2_overall, pouch2_details = check_pouch_sachet(
                        pouch2_lines, product_type, market_type, expected_mfg, expected_line2, expected_exp
                    )
                else:
                    ai_time2 = pouch2_json.get("time", "")
                    pouch2_overall, pouch2_details = check_pouch_linapack(
                        pouch2_lines, product_type, market_type, expected_mfg, expected_line2, expected_exp, mix_code, ai_time2
                    )
            ...
            for d in pouch_details:
                d = dict(d)
                d["item"] = "ซองเครื่อง 1 - " + str(d.get("item", ""))
                details.append(d)
            for d in pouch2_details:
                d = dict(d)
                d["item"] = "ซองเครื่อง 2 - " + str(d.get("item", ""))
                details.append(d)
            ...
            overall = bool(pouch_overall and pouch2_overall and carton_overall)
            check_type_name = "POUCH + CARTON"
            lines = {"pouch1": pouch_lines, "pouch2": pouch2_lines, "carton": carton_lines}
        ...
        if check_type == "both" and expected_line2:
            expected_pouch_lot += " | เครื่อง 2: " + build_expected_pouch(expected_line2)
