...
def linapack_requires_mix(product_type, market_type, carton_alpha_code=""):
    product_type = str(product_type or "").upper()
    market_type = str(market_type or "").upper()
    # VN-MT / Prefix VT does NOT require Mix Code.
    # Standard rule: only EPW Thailand/Laos uses the mix-date code.
    return product_type == "EPW" and market_type in ["TH", "LAOS"]



def linapack_requires_exp(product_type, market_type, carton_alpha_code=""):
    product_type = str(product_type or "").upper()
    market_type = str(market_type or "").upper()
    carton_alpha_code = str(carton_alpha_code or "").strip().upper()
    # VN-MT / Prefix VT pouch must verify EXP even though it is an export job.
    if market_type == "EXPORT" and carton_alpha_code == "VT":
        return True
...
def expected_linapack_exp(product_type, market_type, expected_mfg, epc_laos_exp_months=24, carton_alpha_code=""):
    product_type = str(product_type or "").upper()
    market_type = str(market_type or "").upper()
    carton_alpha_code = str(carton_alpha_code or "").strip().upper()

    # VN-MT / Prefix VT pouch includes EXP.
    if market_type == "EXPORT" and carton_alpha_code == "VT":
        if product_type == "EPC":
...
def no_exp_required(product_type, market_type, carton_alpha_code=""):
    """
    True = ไม่ต้องมี EXP
    False = ต้องมี EXP
    ...
    EXPORT Prefix VT / VN-MT : ไม่ตรวจวันผสม และ Unit/Pouch ต้องตรวจ EXP
    ...
    """
    return not linapack_requires_exp(product_type, market_type, carton_alpha_code)
...
def _deepseek_chat_completion(content, timeout=180):
    """
    Call DeepSeek Chat Completions directly over HTTPS.
    `content` is a multimodal list of text and image_url blocks.
    """
    if not DEEPSEEK_API_KEY:
        raise RuntimeError("ไม่พบ DEEPSEEK_API_KEY")
    ...
    try:
        ...
        raise RuntimeError(f"DeepSeek API HTTP {exc.code}: {body[:1200]}") from exc
    except Exception as exc:
        raise RuntimeError(f"เชื่อมต่อ DeepSeek API ไม่สำเร็จ: {exc}") from exc

    try:
        data = json.loads(response_text)
    except Exception as exc:
        raise RuntimeError("DeepSeek API ส่งข้อมูลกลับมาไม่ใช่ JSON: " + response_text[:500]) from exc

    try:
        message = data["choices"][0]["message"]["content"]
    except Exception as exc:
        raise RuntimeError("DeepSeek API response ไม่มี choices[0].message.content: " + response_text[:500]) from exc
    ...
    raise RuntimeError("DeepSeek API ไม่ได้ส่งข้อความ OCR กลับมา")


def _deepseek_image_block(image_base64, check_type="", detail="original"):
    mime_type = _detect_image_mime_type(image_base64)
    enhanced = _extract_base64_payload(
        enhance_lot_image_for_ai(image_base64, check_type)
    )
    return {
        "type": "image_url",
        "image_url": {
            "url": f"data:{mime_type};base64,{enhanced}",
            "detail": detail,
        },
    }
...
def get_red_boxes_with_ai(image_base64, details, lines):
    """
    Ask DeepSeek Vision to locate the printed lot area that caused NG.
...
"""
        ...
        result_text = _deepseek_chat_completion([
            {"type": "text", "text": prompt},
            _deepseek_image_block(image_base64, "carton", detail="original"),
        ])
...
def call_deepseek_vision_ocr(image_base64, prompt, check_type):
    """Call DeepSeek Vision OCR and return raw JSON text."""
    return _deepseek_chat_completion([
        {"type": "text", "text": prompt},
        _deepseek_image_block(image_base64, check_type, detail="original"),
    ])
...
PUNCTUATION RULES:
- Preserve visible punctuation exactly, especially hyphen "-".
- If the image shows VN-MT, return VN-MT exactly. Do NOT return VNMT, VN MT, or VN–MT.
- A hyphen is part of the Shipping Mark and must not be dropped.
...
"""
    ...
    return call_deepseek_vision_ocr(image_base64, prompt, check_type)
...
- Preserve visible punctuation exactly, especially hyphen "-".
- If the carton shows VN-MT, transcribe VN-MT exactly; do not collapse it to VNMT or VN MT.
...
def call_deepseek_batch_ocr(image_items, prompt):
    """One DeepSeek Vision request for all pouch/carton images."""
    content = [{"type": "text", "text": prompt}]
    for item in image_items:
        content.append({
            "type": "text",
            "text": f"IMAGE_ID: {item['id']} ({item.get('check_type','')})",
        })
        content.append(
            _deepseek_image_block(
                item["image"],
                item.get("check_type", ""),
                detail="original",
            )
        )
    return _deepseek_chat_completion(content)
...
def read_both_batch_with_ai(pouches, carton_image_base64, product_type, market_type, mode):
    ...
    raw = call_deepseek_batch_ocr(image_items, prompt)
    parsed = _safe_json_loads_from_ai(raw)
    parsed["ocr_batch_provider"] = "deepseek"
...
def check_pouch_sachet(lines, product_type, market_type, expected_mfg, expected_line, expected_exp, carton_alpha_code=""):
    """
    ...
    details = []
    overall = True
    skip_exp = no_exp_required(product_type, market_type, carton_alpha_code)
...
def check_pouch_auto(lines, market_type, expected_mfg, expected_line, expected_exp, carton_alpha_code=""):
    """
...
def check_pouch_linapack(lines, product_type, market_type, expected_mfg, expected_line, expected_exp, mix_code, ai_time="", epc_laos_exp_months=24, carton_alpha_code=""):
    """
    ...
    require_mix = linapack_requires_mix(product_type, market_type, carton_alpha_code)
    require_exp = linapack_requires_exp(product_type, market_type, carton_alpha_code)

    expected_exp_calc = expected_linapack_exp(product_type, market_type, expected_mfg, epc_laos_exp_months, carton_alpha_code)
    if expected_exp_calc:
        expected_exp = expected_exp_calc
        # LINAPACK_RULE_EXPECTED_EXP_OVERRIDE
        try:
            if str(mode).lower() == "linapack":
                expected_exp = expected_linapack_exp(product_type, market_type, expected_mfg, epc_laos_exp_months, carton_alpha_code)
        except Exception:
            pass
...
def extract_carton_actual_fields(all_text, expected_mfg="", carton_alpha_code="", shipping_mark="", building_no="", building_suffix=""):
    ...
    """
    text = normalize(all_text)
    # Keep raw tokens for punctuation-sensitive fields such as VN-MT.
    # Structural parsing still uses compact A-Z/0-9 tokens.
    ...
    raw_tokens = [t.strip().upper() for t in text_for_punctuation.split() if t.strip()]
    tokens = [clean_lot_token(t) for t in raw_tokens]
    ...
    if shipping_mark_required and run_index is not None and run_index > 0:
        # Preserve visible punctuation. For VT this keeps VN-MT instead of VNMT.
        if run_index - 1 < len(raw_tokens):
            result["shipping_mark"] = raw_tokens[run_index - 1]
        else:
...
def strict_field_status(actual, expected):
    def _norm_visible_punctuation(value):
        return (str(value or "").strip().upper()
                ...
                .replace("‐", "-")
                .replace("‑", "-"))
    actual = _norm_visible_punctuation(actual)
    expected = _norm_visible_punctuation(expected)
...
def check_carton(lines, market_type, expected_mfg, expected_exp, building_no, building_suffix, shipping_mark, carton_alpha_code, ai_json, check_exp=True):
    ...
    if clean_lot_token(carton_alpha_code) in {'OL', 'OD'}:
        return check_carton_ol_od(
            lines, expected_mfg, expected_exp, building_no, building_suffix,
            shipping_mark, carton_alpha_code, check_exp=check_exp
        )
    ...
    if not check_exp:
        ...
        details.append({
            ...
            "expected": "ไม่ตรวจ EXP สำหรับ Work Order นี้"
        })
    elif expected_exp:
...
def friendly_error_message(err):
    ...
    if "deepseek" in low and ("api" in low or "key" in low or "http" in low):
        return "DeepSeek OCR ใช้งานไม่สำเร็จ กรุณาตรวจสอบ DEEPSEEK_API_KEY / DEEPSEEK_MODEL และเครดิต API"
    return raw
...
function updateConditionalFields(){
  ...
  const showMix = mode === "linapack" && (product === "EPW" && (market === "TH" || market === "LAOS"));
  ...
  const hideExp = product === "EPW" && (market === "TH" || (market === "EXPORT" && prefix !== "VT"));
...
function updateCalculated(){
  ...
  if(m === "EXPORT" && (($("cartonPrefix")?.value || "").toUpperCase() === "VT")){ if(p === "EPC") exp=addMonths(d,15); if(p === "EPW") exp=addMonths(d,36); }
...
function getPayload(){
  ...
  return {
    ...
    mfg:$("mfg").value,
    exp:($("productType").value==="EPW" && ($("marketType").value==="TH" || ($("marketType").value==="EXPORT" && $("cartonPrefix").value!=="VT"))) ? "" : $("exp").value,
    mixDate:($("mode").value==="linapack" && $("productType").value==="EPW" && ["TH","LAOS"].includes($("marketType").value)) ? ddmmyyFromDate($("mixDate")?.value || "") : "",
    mixCode:($("mode").value==="linapack" && $("productType").value==="EPW" && ["TH","LAOS"].includes($("marketType").value)) ? $("mixCode").value.trim().toUpperCase() : "",
    cartonAlphaCode:$("cartonPrefix").value,
...
async function saveWO(){
  ...
  const mixRequired = p.mode === "linapack" && p.productType === "EPW" && ["TH","LAOS"].includes(p.marketType);
...
def api_work_orders():
    ...
    # Linapack mix-date validation. VN-MT / Prefix VT does not use Mix Code.
    ...
    wo = {
        ...
        "mixDate": str(payload.get("mixDate", "") or "").strip() if linapack_requires_mix(product_type, market_type, carton_code_input) else "",
        "mixCode": str(payload.get("mixCode", "") or "").strip().upper() if linapack_requires_mix(product_type, market_type, carton_code_input) else "",
...
def enrich_work_order(wo):
    ...
    carton_code_for_rules = str(wo.get("cartonAlphaCode", wo.get("cartonPrefix", "")) or "").strip().upper()
    ...
    if mode == "auto":
        ...
        exp = expected_linapack_exp(product_type, market_type, mfg, epc_months, carton_code_for_rules)
    if not exp:
    ...
    def expected_pouch_for(line_code):
        ...
        if linapack_requires_mix(product_type, market_type, wo.get("cartonAlphaCode", wo.get("cartonPrefix", ""))) and mix_code:
            line1 += f" {mix_code}"
...
def _check_sync_from_data(data):
    try:
        ...
        if not DEEPSEEK_API_KEY:
            return jsonify({"error": "ไม่พบ DEEPSEEK_API_KEY"}), 500
        ...
        if mode == "auto":
            ...
            auto_exp = expected_linapack_exp(product_type, market_type, expected_mfg, epc_laos_exp_months, carton_alpha_code)
            expected_exp = auto_exp if auto_exp else ""
            skip_exp = no_exp_required(product_type, market_type, carton_alpha_code)
            ...
            for idx, pouch_item in enumerate(pouches, start=1):
                ...
                if mode == "sachet":
                    pouch_overall_i, pouch_details_i = check_pouch_sachet(
                        pouch_lines_i, product_type, market_type, expected_mfg, line_code, expected_exp, carton_alpha_code
                    )
                elif mode == "auto":
                    pouch_overall_i, pouch_details_i = check_pouch_auto(
                        pouch_lines_i, market_type, expected_mfg, line_code, expected_exp, carton_alpha_code
                    )
                else:
                    ai_time_i = ""
                    ...
                    pouch_overall_i, pouch_details_i = check_pouch_linapack(
                        pouch_lines_i, product_type, market_type, expected_mfg, line_code, expected_exp, mix_code, ai_time_i, epc_laos_exp_months, carton_alpha_code
                    )
        ...
        if not batch_result_handled and check_type == "carton":
            ...
            overall, details = check_pouch_linapack(
                ...
                mix_code,
                ai_time,
                carton_alpha_code=carton_alpha_code
            )
            mode_name = "Linapack"
            check_type_name = "POUCH"

        # Attach a clear source to every validation result so the operator can
        # immediately distinguish a pouch error from a carton error. Existing
        # multi-image branches already prefix item names; single-image branches
        # are labelled here as well.
        normalized_details = []
        for raw_detail in (details or []):
            detail = dict(raw_detail or {})
            item_text = str(detail.get("item", "") or "").strip()
            pouch_match = re.match(r"^(ซองเครื่อง\s*\d+)\s*-\s*", item_text)
            if pouch_match:
                source_label = pouch_match.group(1)
                source_type = "pouch"
            elif re.match(r"^กล่อง\s*-\s*", item_text):
                source_label = "กล่อง"
                source_type = "carton"
            elif check_type == "carton":
                source_label = "กล่อง"
                source_type = "carton"
                detail["item"] = f"กล่อง - {item_text}" if item_text else "กล่อง - รายการตรวจ"
            else:
                source_label = "ซองเครื่อง 1"
                source_type = "pouch"
                detail["item"] = f"ซองเครื่อง 1 - {item_text}" if item_text else "ซองเครื่อง 1 - รายการตรวจ"
            detail["source"] = source_type
            detail["sourceLabel"] = source_label
        ...
        def build_expected_pouch(line_code):
            ...
            if linapack_requires_mix(product_type, market_type, carton_alpha_code) and mix_code:
                line1 += f" {mix_code}"
